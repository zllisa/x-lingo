...
import { aiTranslate, aiTranslateBatch, aiResegment } from './ai/tasks';
...
export async function transcribeFile(
  ...
  const segments = await resegmentWords(words, aiResegment, onProgress);
  if (!segments.length) {
  ...
  for (let i = 0; i < segments.length; i += BATCH_SIZE) {
    ...
    try {
      translations = await aiTranslateBatch(batch.map(s => s.text));
    } catch (e: any) {
      ...
      translations = await Promise.all(
        batch.map(s => aiTranslate(s.text).catch(() => '(翻译失败)')),
      );
    }
